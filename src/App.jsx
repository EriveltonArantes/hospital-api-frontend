...
const apiBaseUrl = "https://hospital-api.onrender.com";
    ...
    <div className="login-screen">
      <div className="login-institucional">
        ...
        <ul className="login-features">
          ...
          <li>✓ Controle completo de Medico</li>
          <li>✓ Controle completo de Leito</li>
          <li>✓ Controle completo de Internacao</li>
        </ul>
      </div>
...
function PainelPaciente({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  const itensFiltrados = itens.filter(item => !busca || ["nome", "cpf", "dataNascimento", "email", "telefone", "planoSaude", "tipoSanguineo", "alergias", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "cpf", "dataNascimento", "email", "telefone", "planoSaude", "tipoSanguineo", "alergias"].forEach(k => { f[k] = item[k] ?? ""; });
  ...
  return (
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-field"><b>planoSaude</b><span>{item["planoSaude"] ?? "—"}</span></div>
            <div className="item-field"><b>tipoSanguineo</b><span>{item["tipoSanguineo"] ?? "—"}</span></div>
            <div className="item-field"><b>alergias</b><span>{item["alergias"] ?? "—"}</span></div>
      ...
      <label className="field-label">planoSaude</label>
      <input type="text" value={form["planoSaude"] ?? ""} onChange={e => setForm({...form, planoSaude: e.target.value})} />
      <label className="field-label">tipoSanguineo</label>
      <select value={form["tipoSanguineo"] ?? "A+"} onChange={e => setForm({...form, tipoSanguineo: e.target.value})}>
      ...
      <input type="text" value={form["alergias"] ?? ""} onChange={e => setForm({...form, alergias: e.target.value})} />
              <div className="modal-actions">
...
function PainelMedico({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["nome", "crm", "especialidade", "email", "telefone", "disponivel", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/medicos", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/medicos/" + editId : apiBaseUrl + "/api/medicos";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "crm", "especialidade", "email", "telefone", "disponivel"].forEach(k => { f[k] = item[k] ?? ""; });
    [].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/medicos/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["nome"] ?? "Medico")}</div>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Medico</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">nome</label>
      <input type="text" value={form["nome"] ?? ""} onChange={e => setForm({...form, nome: e.target.value})} />
...
function PainelLeito({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["numero", "ala", "tipo", "status", "descricao", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/leitos", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/leitos/" + editId : apiBaseUrl + "/api/leitos";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["numero", "ala", "tipo", "status", "descricao"].forEach(k => { f[k] = item[k] ?? ""; });
    [].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/leitos/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
            ...
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["ala"] ?? "Leito")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
                    fetch(apiBaseUrl + "/api/leitos/" + item.id, {
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["ala"] ?? "Leito")}</div>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Leito</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
      ...
      <label className="field-label">ala</label>
      <input type="text" value={form["ala"] ?? ""} onChange={e => setForm({...form, ala: e.target.value})} />
...
function PainelInternacao({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  const [leitoList, setLeitoList] = React.useState([]);
  const [medicoList, setMedicoList] = React.useState([]);

  // Paginação real (achado real auditando como cliente exigente: lista sem
  // limite nenhum, "queria ver pelo menos os 10 primeiros"). Front-end só —
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["pacienteId", "medicoId", "leitoId", "dataEntrada", "dataSaida", "diagnostico", "status", "observacoes", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/internacaos", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  React.useEffect(() => {
    ...
    fetch(apiBaseUrl + "/api/leitos", { headers: authHeaders(token) }).then(r => r.json()).then(setLeitoList).catch(() => {});
    fetch(apiBaseUrl + "/api/medicos", { headers: authHeaders(token) }).then(r => r.json()).then(setMedicoList).catch(() => {});
  }, [token]);
  ...
  const montarCorpo = () => {
    const corpo = { ...form };
    ["paciente", "medico", "leito"].forEach(k => {
      corpo[k + "Id"] = corpo[k] ? Number(corpo[k]) : null;
      delete corpo[k];
    });
    // Bug real (2026-06-26): <input type="datetime-local"> envia "YYYY-MM-DDTHH:mm"
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/internacaos/" + editId : apiBaseUrl + "/api/internacaos";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["pacienteId", "medicoId", "leitoId", "dataEntrada", "dataSaida", "diagnostico", "status", "observacoes"].forEach(k => { f[k] = item[k] ?? ""; });
    ["paciente", "medico", "leito"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/internacaos/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
        ...
        <div className="kanban-board">
          {[...new Set(itensFiltrados.map(i => String(i.status ?? "Sem status")))].map(coluna => (
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["diagnostico"] ?? "Internacao")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
                    fetch(apiBaseUrl + "/api/internacaos/" + item.id, {
                      method: "PUT", headers: { "Content-Type": "application/json", ...authHeaders(token) },
                      body: JSON.stringify({ ...item, status: e.target.value }),
                    }).then(carregar);
                  }}>
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["diagnostico"] ?? "Internacao")}</div>
            <span className={"status-pill " + corStatus(item.status)}>{String(item.status ?? "")}</span>
            <div className="item-meta-grid">
            <div className="item-field"><b>pacienteId</b><span>{item["pacienteId"] ?? "—"}</span></div>
            <div className="item-field"><b>medicoId</b><span>{item["medicoId"] ?? "—"}</span></div>
            <div className="item-field"><b>leitoId</b><span>{item["leitoId"] ?? "—"}</span></div>
            ...
            <div className="item-field"><b>medico</b><span>{item.medicoId ? ((medicoList.find(o => o.id === item.medicoId) || {}).nome ?? ("#" + item.medicoId)) : "—"}</span></div>
            <div className="item-field"><b>leito</b><span>{item.leitoId ? ((leitoList.find(o => o.id === item.leitoId) || {}).ala ?? ("#" + item.leitoId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
              {role === "ADMIN" && <button className="btn btn-danger btn-sm" onClick={() => { if(window.confirm("Confirmar exclusão?")) apagar(item.id); }}>Apagar</button>}
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Internacao</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
      ...
      <label className="field-label">medicoId</label>
      <input type="number" value={form["medicoId"] ?? ""} onChange={e => setForm({...form, medicoId: e.target.value})} />
      <label className="field-label">leitoId</label>
      <input type="number" value={form["leitoId"] ?? ""} onChange={e => setForm({...form, leitoId: e.target.value})} />
      ...
      <label className="field-label">medico</label>
      <select value={form["medico"] ?? ""} onChange={e => setForm({...form, medico: e.target.value})}>
        <option value="">Selecione...</option>
        {(medicoList || []).map(o => (<option key={o.id} value={o.id}>{o.nome ?? ("#" + o.id)}</option>))}
      </select>
      <label className="field-label">leito</label>
      <select value={form["leito"] ?? ""} onChange={e => setForm({...form, leito: e.target.value})}>
        <option value="">Selecione...</option>
        {(leitoList || []).map(o => (<option key={o.id} value={o.id}>{o.ala ?? ("#" + o.id)}</option>))}
      </select>
              <div className="modal-actions">
                <button type="submit" className="btn btn-primary" disabled={salvando}>{salvando ? "Salvando..." : "Salvar"}</button>
                <button type="button" className="btn btn-ghost" onClick={() => setModalAberto(false)} disabled={salvando}>Cancelar</button>
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="sidebar-logo">Hospital<span className="dot">.</span></div>
        <button className={"nav-btn" + (aba === "Paciente" ? " active" : "")} onClick={() => setAba("Paciente")}><span className="nav-ico">👤</span>Paciente</button>
        <button className={"nav-btn" + (aba === "Medico" ? " active" : "")} onClick={() => setAba("Medico")}><span className="nav-ico">🗂️</span>Medico</button>
        <button className={"nav-btn" + (aba === "Leito" ? " active" : "")} onClick={() => setAba("Leito")}><span className="nav-ico">🗂️</span>Leito</button>
        <button className={"nav-btn" + (aba === "Internacao" ? " active" : "")} onClick={() => setAba("Internacao")}><span className="nav-ico">🗂️</span>Internacao</button>
      ...
      {aba === "Medico" && <PainelMedico token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "Leito" && <PainelLeito token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "Internacao" && <PainelInternacao token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      </main>
